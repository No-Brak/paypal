...
def home():
    return render_template('index.html', 
                          channel='https://t.me/NO_BRAK', 
                          owner='https://t.me/c249c',
                          dev='WOODE')
...
def test_single():
    ...
    try:
        ...
        if result['status'] in ['Charged 🔥', 'good']:
            test_results['good'].append({
                'card': f"{n[:6]}XXXXXX{n[-4:]}",
                'expiry': f"{mm}/{yy}",
                'result': result
            })
        else:
            test_results['bad'].append({
                'card': f"{n[:6]}XXXXXX{n[-4:]}",
                'expiry': f"{mm}/{yy}",
                'result': result
            })
...
@app.route('/test-group', methods=['POST'])
def test_group():
    ...
    try:
        ...
        with open(filepath, 'r') as f:
            cards = [line.strip() for line in f.readlines() if '|' in line]
        ...
        return jsonify({
            ...
            'good_cards': len(test_results['good']),
            'bad_cards': len(test_results['bad']),
            'results': results
        })
...
def get_results():
    ...
    return jsonify({
        ...
        'total_good': len(test_results['good']),
        'total_bad': len(test_results['bad'])
    })
